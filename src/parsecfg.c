...
/* $Id: parsecfg.c,v 1.1.1.1 2008-07-26 21:19:22 rocco Exp $ */
...
static void cfgFatalFunc(cfgErrorCode error_code, const char *file, int line, const char *str)
{
	...
	case CFG_JUST_RETURN_WITHOUT_MSG:
		break;
/* 
 * Per gestire valori non inizializzati nel file di configurazione (VP) esempio:
 * FileLog=
*/
	case CFG_NO_VALUE:
		fprintf(stderr, _("%s(%d): %s\nThere is no value associated to the parameter.\n"), file, line, str);
		break;
	default:
		fprintf(stderr, _("%s(%d): %s\nUnexplained error\n"), file, line, str);
	}
...
static int store_value(cfgStruct cfg[], const char *parameter, const char *value, cfgFileType type, int section)
{
	...
	for (num = 0; cfg[num].type != CFG_END; num++) {
		if (strcasecmp(parameter, cfg[num].parameterName) == 0) {
			/* 
			 * Per gestire valori non inizializzati nel file di
			 * configurazione (VP) esempio:
			 * FileLog=
			 */
			if(!*value) {
				return (CFG_NO_VALUE);
		...
		case CFG_UINT:
			...
			*(int **) (cfg[num].value) = ptr;
			if (cfg[num].type == CFG_BOOL) {
			  ...
			  *(*((int **) (cfg[num].value)) + *section) = -1;  /* NOI!!! */
			}
			break;
	...
	while ((ptr = get_single_line_without_first_spaces(fp, &line_buf, &line)) != NULL) {	/* malloc line_buf */
		if (*ptr == '[') {
			...
			if ((section_num > 0 && current_section_number == section_num) || 
			    (section_num <= 0 && strcasecmp(read_section_name, section_name) == 0)) {
				section_flag = 0;
				free(line_buf);
				free(read_section_name);
				continue;
